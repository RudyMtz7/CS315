...
var t_ball, tlabel, li1, li2, plabel;
var curve, curveGeometry, curveObject;
var v0, v1, v2, v0label, v1label, v2label;
var interLineGeo;
var handle, handleGeo;
var interBallA, interBallB, interLine;
...
function drawBezierCurve() {
    // BEGIN CHANGES
    var controlGeometry = new THREE.SphereGeometry(30, 12, 12);
    var interGeo = new THREE.SphereGeometry(20, 12, 12);
    ...
    var greenMat = new THREE.MeshLambertMaterial({ color: 0x55ff22 });
    var handleMat = new THREE.LineBasicMaterial({ color: 0xff0000 });
    var interMat = new THREE.LineBasicMaterial({ color: 0xffff00 });
    var lineMat = new THREE.LineBasicMaterial({
        linewidth: 3,
        color: 0x000000
    });
    v0 = new THREE.Mesh(controlGeometry, blackMat);
    v1 = new THREE.Mesh(controlGeometry, redMat);
    v2 = new THREE.Mesh(controlGeometry, blackMat);
    t_ball = new THREE.Mesh(controlGeometry, greenMat);
    v0.position.x = 0;
    v0.position.y = 0;
    v0.position.z = -200;
    v1.position.x = 0;
    ...
    t_ball.position.y = 0;
    t_ball.position.z = 0;
    handleGeo = new THREE.Geometry();
    handleGeo.vertices.push(
        v0.position,
        v1.position,
        v2.position
    );
    handle = new THREE.Line(handleGeo, handleMat);
    ...
    curveObject = new THREE.Line(curveGeometry, lineMat);
    interBallA = new THREE.Mesh(interGeo, yellowMat);
    interBallA.position.x = 0;
    interBallA.position.y = 0;
    interBallA.position.z = 0;
    interBallB = new THREE.Mesh(interGeo, yellowMat);
    interBallB.position.x = 0;
    interBallB.position.y = 0;
    interBallB.position.z = 0;
    interLineGeo = new THREE.Geometry();
    interLineGeo.vertices.push(
        interBallA.position,
        interBallB.position
    );
    interLine = new THREE.Line(interLineGeo, interMat);
    scene.add(v0);
    scene.add(v1);
    scene.add(v2);
    scene.add(t_ball);
    scene.add(handle);
    scene.add(interBallA);
    scene.add(interBallB);
    scene.add(interLine);
...
function setTBallPosition() {
    ...
    interBallA.position.set(0, 0, 0).add(p01);
    interBallB.position.set(0, 0, 0).add(p12);
    t_ball.position.set(0, 0, 0).add(p012);
    interLineGeo.vertices = [interBallA.position, interBallB.position];
    interLineGeo.verticesNeedUpdate = true;
    // END CHANGES
}
...
function onDocumentMouseMove(event) {
    ...
    if (SELECTED) {
        if (raycaster.ray.intersectPlane(plane, intersection)) {
            ...
            handleGeo.vertices = [v0.position, v1.position, v2.position];
            handleGeo.verticesNeedUpdate = true;
            interLineGeo.vertices = [interBallA.position, interBallB.position];
            interLineGeo.verticesNeedUpdate = true;
            // END CHANGES
        }
